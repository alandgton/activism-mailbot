...
def saytheirnames_sublist(num, names):
    if num == 0:
        return ""
    elif num == 1:
        ...
        names.remove(name)
        return name + ", " + saytheirnames_sublist(num-1, names)
...
def gen_subject(): #note can this function be used later if its part of the app route? (im very new to flask)
    ...
    if random.randint(1,100) > percent_for_list:
        ...
        return "George Floyd, " + saytheirnames_sublist(4, saytheirnames_list) + random.choice(suffix)

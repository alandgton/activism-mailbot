...
# Generates a list of n names killed by police #saytheirnames
def saytheirnames_sublist(n):
    if n == 0:
        return ""
    elif n == 1:
        return random.choice(saytheirnames_list)
    else:
        return random.choice(saytheirnames_list) + ", " + saytheirnames_sublist(n-1)
...
def gen_subject(): #note can this function be used later if its part of the app route? (im very new to flask)
    ...
    if random.randint(1,100) > percent_for_list:
        ...
        return "George Floyd, " + saytheirnames_sublist(5) + random.choice(["...","...and Many More","...and So Many More","...and Many Others","...and So Many Others"])
